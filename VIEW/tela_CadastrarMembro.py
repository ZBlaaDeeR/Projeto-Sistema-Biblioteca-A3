...
class tela_CadastrarMembro:
    def __init__(self,root_ref,IDfuncionario=None):
        ...
        # Cria uma nova janela secundária 
          self.janelaCadastroMembro = tk.Toplevel()
          self.janelaCadastroMembro.title("Cadastrar Membros")
          largura = 1024
          altura = 768
        
          #Posicionamento do programa(Ao inicia-lo)
          largura_tela = self.janelaCadastroMembro.winfo_screenwidth()
          altura_tela = self.janelaCadastroMembro.winfo_screenheight()
          x = (largura_tela - largura) // 2
          y = (altura_tela - altura) // 2
          self.janelaCadastroMembro.geometry(f"{largura}x{altura}+{x}+{y}")
          ...
          self.label_imagem = tk.Label(self.janelaCadastroMembro, image=self.imagem_tk)
          self.label_imagem.image = self.imagem_tk
          self.label_imagem.place(x=0, y=0,relwidth=1, relheight=1)

          #Frame
          self.frame = tk.Frame(self.janelaCadastroMembro, width=678, height=315)
          self.frame.place(x=170,y=220)
          self.frame2 = tk.Frame(self.janelaCadastroMembro, width=300, height=50)
          ...
          self.b_Salvar = tk.Button( self.frame,text='SALVAR',width=12,height=2,command=self.SalvarMembro,relief="raised", overrelief="ridge",bg="light blue",fg="black")
          self.b_Salvar.place(x=570,y=240)

          #Codigo para impedir manipulação da tela
          self.janelaCadastroMembro.resizable(False, False)
    ...
    def SalvarMembro(self):
         ...
         if not nome or not gmail or not telefone or not cpf:
               messagebox.showwarning("Campos Obrigatórios", "Preencha todos os campos antes de Cadastrar o membro.")
               return
         
         if not membro.verificarTelefone(telefone):
               messagebox.showwarning("Telefone inválido", "O telefone deve conter pelo menos 15 caracteres.")
               return
                    
         if not membro.validadorCpf(cpf):
               messagebox.showerror("CPF Inválido", "O CPF informado não é válido.")
               return

         membro.cadastrarMembro(nome,gmail,telefone,cpf)



    def onClose(self):
        self.janelaCadastroMembro.destroy()
